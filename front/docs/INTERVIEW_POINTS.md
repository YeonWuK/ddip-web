...
**최신 상태 (2026년 2월)**: 백엔드 API 연동 완료. 프로젝트/경매/인증/사용자/배송지 API는 실제 서버와 연동됨.
...
### 1. 에러/로딩 경계 (error.tsx, loading.tsx)
- Next.js App Router의 파일 기반 경계 사용
- 전역 + 프로젝트/경매 상세 세그먼트에 분리 적용
- 에러 시 `reset()` 버튼으로 재시도, 상세 페이지는 "목록으로 이동" 제공
- 역할 분리: error.tsx는 렌더 throw, try-catch는 API 실패
...
### Q4. 에러 처리는 어떻게 했나요?
**답변:**
- Next.js App Router의 error.tsx/loading.tsx로 라우트별 에러·로딩 경계 적용
- try-catch로 API 호출 에러 처리
- null/undefined 안전 처리 (옵셔널 체이닝, 기본값 사용)
- 날짜 유효성 검사
- 에러 시 reset() 버튼으로 재시도 가능
